...
import com.zest.toeic.productivity.repository.StudyPlanRepository;
...
class StudyPlanServiceTest {
    ...
    void generatePlan_noExisting() {
        when(repository.findByUserIdAndStatus("u1", GoalStatus.ACTIVE.name())).thenReturn(Optional.empty());
    ...
    void generatePlan_expiresOldPlan() {
        StudyPlan old = StudyPlan.builder().userId("u1").status(GoalStatus.ACTIVE).build();
        when(repository.findByUserIdAndStatus("u1", GoalStatus.ACTIVE.name())).thenReturn(Optional.of(old));
        when(repository.save(any())).thenAnswer(i -> i.getArgument(0));

        service.generatePlan("u1", 6.0, List.of("Vocabulary"));
        assertEquals(GoalStatus.EXPIRED, old.getStatus());
    }
    ...
    @Test
    void getCurrentPlan() {
        StudyPlan plan = StudyPlan.builder().userId("u1").status(GoalStatus.ACTIVE).build();
        when(repository.findByUserIdAndStatus("u1", GoalStatus.ACTIVE.name())).thenReturn(Optional.of(plan));
    ...
    void completeTask_success() {
        List<StudyPlan.PlanTask> tasks = new ArrayList<>(List.of(
                StudyPlan.PlanTask.builder().type(PlanTaskType.PRACTICE).durationMinutes(30).build()));
    ...
    void adjustPlan_redistributes() {
        ...
        List<StudyPlan.PlanTask> day1Tasks = new ArrayList<>(List.of(
                StudyPlan.PlanTask.builder().type(PlanTaskType.PRACTICE).durationMinutes(30).completed(false).build()));
        List<StudyPlan.PlanTask> day7Tasks = new ArrayList<>(List.of(
                StudyPlan.PlanTask.builder().type(PlanTaskType.REVIEW).durationMinutes(20).build()));
        List<StudyPlan.DailyPlan> dailyPlans = new ArrayList<>(List.of(
