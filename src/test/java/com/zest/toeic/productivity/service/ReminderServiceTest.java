...
class ReminderServiceTest {
    ...
    @Test
    void create() {
        Reminder r = Reminder.builder().type(ReminderType.DAILY_STUDY).schedule("07:30").message("Time to learn!").build();
    ...
    @Test
    void update_success() {
        Reminder existing = Reminder.builder().type(ReminderType.DAILY_STUDY).schedule("07:30").message("old").build();
        existing.setId("r1");
        when(repository.findById("r1")).thenReturn(Optional.of(existing));
        when(repository.save(any())).thenAnswer(i -> i.getArgument(0));

        Reminder update = Reminder.builder().type(ReminderType.CUSTOM).schedule("08:00").message("new").enabled(true).build();
    ...
    void processReminders_sendsNotifications() {
        ...
        verify(notificationService).send(eq("u1"), eq(NotificationType.REMINDER), anyString(), eq("Time!"), any());
    }
    ...
    void processReminders_respectsQuietHours() {
        ...
        service.processReminders();
        verify(notificationService, never()).send(anyString(), any(NotificationType.class), anyString(), anyString(), any());
    }
