...
class BattleRestControllerTest {
    ...
    void getBattle() throws Exception {
        Battle b = Battle.builder().title("Test").status("SCHEDULED").build();
        b.setId("b1");
    ...
    void startBattle_broadcastsStart() throws Exception {
        Battle b = Battle.builder().status("IN_PROGRESS").build();
    ...
    void endBattle_broadcastsEnd() throws Exception {
        Battle b = Battle.builder().status("COMPLETED").build();
