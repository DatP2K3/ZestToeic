...
class BattleServiceTest {
    ...
    void register_success() {
        Battle battle = Battle.builder().status("SCHEDULED").maxPlayers(30).build();
        battle.setId("b1");
    ...
    void register_battleFull() {
        Battle battle = Battle.builder().status("SCHEDULED").maxPlayers(2).build();
        battle.setId("b1");
        when(battleRepository.findById("b1")).thenReturn(Optional.of(battle));
    ...
    void register_battleNotOpen() {
        Battle battle = Battle.builder().status("COMPLETED").build();
        battle.setId("b1");
        when(battleRepository.findById("b1")).thenReturn(Optional.of(battle));
        assertThrows(IllegalStateException.class, () -> service.register("b1", "u1", "User 1"));
    }
    ...
    @Test
    void startBattle() {
        Battle battle = Battle.builder().status("SCHEDULED").build();
        battle.setId("b1");
        when(battleRepository.findById("b1")).thenReturn(Optional.of(battle));
    ...
    @Test
    void endBattle() {
        Battle battle = Battle.builder().status("IN_PROGRESS").build();
        battle.setId("b1");
        when(battleRepository.findById("b1")).thenReturn(Optional.of(battle));
        when(battleRepository.save(any())).thenAnswer(i -> i.getArgument(0));
